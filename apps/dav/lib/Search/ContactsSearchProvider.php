...
class ContactsSearchProvider implements IProvider {
	...
	public function search(IUser $user, ISearchQuery $query): SearchResult {
		...
		$formattedResults = \array_map(function(array $contactRow) use($addressBooksById):ContactsSearchResultEntry {
			...
			if ($vCard->PHOTO) {
				$thumbnailUrl = $this->getDavUrlForContact($addressBook, $contactRow) . '?photo';
			} else {
				$thumbnailUrl = $this->urlGenerator->imagePath('contacts', 'app.svg');
			}
			...
			if (is_array($emailAddresses) && count($emailAddresses) > 0) {
				$subline = (string)$emailAddresses[0];
			}

			$resourceUrl = $this->getDeepLinkToContactsApp($addressBook['uri'], (string) $vCard->UID);

			return new ContactsSearchResultEntry($thumbnailUrl, $title, $subline, $resourceUrl);
		}, $searchResults);
	 ...
	 * @param array $contactRow
	 * @return string
	 */
	private function getDavUrlForContact(array $addressBook, array $contactRow): string {
		[, $principalType, $principalId] = explode('/', $addressBook['principaluri'], 3);
	...
	private function getDeepLinkToContactsApp(string $addressBookUri, string $contactUID): string {
		return $this->urlGenerator->getAbsoluteURL(
